...
- **Release-tag grammar (samohost `src/commands/app.ts:1210`):** `^v(\d{4})(\d{2})(\d{2})\.([1-9]\d*)$` — e.g. `v20260904.1`. A tag deploys only if it is **strictly newer** than the last deployed tag, its SHA is an **ancestor of `main`**, and **CI is green on that exact SHA** (`.github/workflows/ci.yml`, pinned by `releaseCiWorkflow` in `.samohost.toml`). Old npm tags (`v0.x`) never match and are ignored. Cut tags with `.github/workflows/release.yml`; operator checklist in `docs/runbooks/deploy-release.md`.
