...
export interface WhisperSink {
  deliver(w: Whisper): void | Promise<void>;
}
...
/**
 * A sink that prints each whisper to stderr — stdout stays the command's own
 * result, so a whisper never pollutes a piped `samograph` invocation. This is
 * the zero-hardware default: the wearer is whoever is reading the terminal.
 */
...
/** A {@link WhisperSink} that keeps every frame it rendered. */
export interface FakeHudSink extends WhisperSink {
  deliver(w: Whisper): void;
  /** Every frame delivered so far, oldest first. */
  frames: HudFrame[];
  /** The most recent frame, or null before the first delivery. */
  lastFrame(): HudFrame | null;
}

/**
 * A sink that renders onto the simulated screen and RETURNS the frames, so a
 * test can assert the exact pixels-worth of text and a human can see the box.
 * Defaults to the real G2 geometry.
 */
