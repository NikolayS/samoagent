...
**Explicit non-goals for v1 (do not silently re-introduce):** no calendar integration, no Google OAuth *(reversed post-v1 — see the amendment note below)*, no AI features in the product UI, no billing, no plans/seats, no video storage, no AI-generated summaries, no email digests, no MS Teams, no consistent/branded bot identity, no multi-language UI. The Phase-2 AI-agent channel is designed for but **not built** in v1; only the **seams** ship in v1 (per-call token model, command/act API in the bot worker, per-call auth, capability scoping, audit log).

> **Amendment (S5-1).** The "no Google OAuth" non-goal above was **reversed post-v1** — deliberately, in writing, with owner sign-off (issue **#209**). Sign up / sign in with Google (OIDC) ships as a **second** credential path and never the only one: magic link stays enabled on every environment where Google is enabled, so a revoked, suspended or deleted Google account always leaves a working way back into the tenant. **The calendar non-goal is NOT reversed** — sign-in requests `openid email` only, and the §5.12 v2 "calendar auto-join" row must request calendar scopes **incrementally from Settings**, never bundled into sign-in. Every other non-goal in this paragraph stands. See `SPEC.amendments.md` → S5-1 (which also amends §2, §5.1, §5.11, §5.12, §5.14, §5.16 and §9).

## 2. Scope phases (authoritative)

### v1 — MVP (the entire build target, this week)
- Marketing + app at `samograph.dev`.
- Email magic-link auth only (passwordless). *(Post-v1: Google (OIDC) sign-in was added alongside magic link — see §1's amendment note and `SPEC.amendments.md` → S5-1. Magic link stays enabled wherever Google is.)*
...
> **Amendment (S5-1).** Post-v1, `GET /auth/google/start` + `GET /auth/google/callback` add Google (OIDC) sign-in alongside the magic link, terminating in the **same** `samo_session` cookie this section mints (one cookie mint site, no second session shape). Identity is keyed on `(provider, provider_subject)` in the privileged `user_identities` table (migration `0011`), **not** on `users.email` — which **no authentication or identity-linking path mutates** (the only `UPDATE` naming `users.email` on any credential path is `apps/app-api/auth/pg-user-store.ts`'s `ON CONFLICT (email) DO UPDATE SET email = EXCLUDED.email`, a provable no-op self-assignment — the conflict target *is* the unique `email` — written so `RETURNING` fires on the conflict branch). That is the load-bearing property: signing in never rewrites an account's address, so a Google account asserting a victim's address cannot move that victim's `users.email` out from under `(provider, provider_subject)` resolution. **Exactly one path deliberately does mutate it — §5.14 erasure** (S5-2 / issue **#220**): account deletion *releases* the address (`apps/app-api/account/http.ts`'s `UPDATE users SET email = …`, backfilled by migration `0013`), rewriting it to the non-routable `deleted-<user id>@deleted.invalid` so a later sign-in provisions a fresh account instead of adopting the erased one. Outside that one path the address is immutable after creation. `email_verified === true` on the locally verified ID token is a hard precondition before any store is touched; linking a Google identity to an existing account is silent and therefore sends a one-time notification email. `GET /auth/providers` **MUST remain** the sole gate on whether the Google button renders on a deployment — it is today, on both the sign-in page and Settings (it reports `{"google": false}` on branch previews, by design). See `SPEC.amendments.md` → S5-1.
...
- A single "activation funnel" dashboard: signup → `auth_completed` → call created → first transcript line → 30s of stream. This IS the v1 success metric (§9).

> **Amendment (S5-1 item 7, issue #222).** Stage 2 is `auth_completed`, renamed from `magic_link_clicked`: the old stage was derived from a consumed `magic_links` row, which a Google signup never produces, and because the funnel is **cumulative** a Google user who created a call had stage 2 back-filled — a magic-link click that never happened, imputed into THE v1 success metric. Arity stays 5, so the cumulative index math is unchanged. `samograph_funnel_stage` and `samograph_activation_w1_by_method` carry a `method` label (`magic_link` | `google`) sourced from `users.signup_method` (migration `0012`, which records account **creation** and is never rewritten by a later sign-in); **per-method series only** — there is deliberately no blended `samograph_funnel_stage{stage="…"}` series, since `sum by (stage)` recovers the blend exactly. Metrics added with it: three **counters** — `auth_google_start_total`, `auth_google_callback_total{result}`, `auth_identity_linked_total` — plus one **gauge**, `samograph_magic_link_status{status}`, which is SET from a periodic read of the `magic_links` lifecycle rather than accumulated (so `rate()` over it is meaningless). `samograph_funnel_stage` and `samograph_activation_w1_by_method` are likewise gauges, rendered from the funnel snapshot.
...
| **Calendar** | Auto-join calendar events | v2 | Connect Google / Microsoft calendar; auto-join rules: only meetings I organize / internal-domain / external / decline-not-accepted / per-event opt-out. Re-introduces calendar (deliberately excluded from v1). Calendar scopes MUST be requested **incrementally from Settings** at the moment the user enables this — never bundled into Google sign-in, which requests `openid email` only (S5-1). |
...
| **Sign-in** | Linked sign-in methods (read-only) | post-v1 (S5-1) | Lists how this account can sign in. `magic_link` is listed unconditionally with the account's authoritative `users.email`; the `google` row reads connected / not connected and is **omitted entirely** where `GET /auth/providers` reports Google is not configured (branch previews) — omitted, not "not connected", because a row for a method that cannot be connected here is an invitation to try. Read-only: connect/disconnect is `[POSTPONED post-v1]`. Never served: `provider_subject`, the provider-asserted `user_identities.email`, or `last_login_at`. |
...
- **Delete account / all tenant data:** purges all of the tenant's `calls`, `transcripts`, `tokens`, `workers`, audit detail, and stored video, **plus the owner's `user_identities` rows** (S5-1: a provider `sub` is personal data, and since the erasure writes an `audit_log` tombstone and never deletes the `users` row the FK cascade never fires; the delete runs on the **privileged** connection, outside the RLS-scoped transaction, because `user_identities` is pre-tenant and ungranted) **and releases the owner's `users.email`**, rewriting it to the deterministic, non-routable `deleted-<user id>@deleted.invalid` in the same privileged transaction (S5-2 / #220: the retained address is itself directly-identifying personal data, and because every sign-in path resolves by address, leaving it in place let the next sign-in re-link a fresh identity to the erased account and email the erased person about it — so **a previously-erased address may be used to sign up again**, and does so as a genuinely fresh account with no history); revokes all sessions; deletes the Recall recordings; emails a confirmation. Honors a **GDPR erasure SLA (≤ 30 days)**, with active calls force-left first.
- **Known gap — the list above is what erasure purges today, not everything it should:** the tenant's `settings` row (keyterms, dictionary, language) is **not** purged and survives an erasure request. Tracked as issue **#233**; recorded here so the SPEC does not imply behaviour that has not shipped.
...
> **Amendment (S5-1).** Failures on the **Google sign-in** path are decided while the browser is mid-redirect between us and `accounts.google.com`: there is no fetch to answer with JSON and no page of ours rendering yet, so they are delivered as `302 Location: /auth?error=<CODE>` and the sign-in page renders the same user-facing string from a shared code→copy map. `SAMO-AUTH-005` and `SAMO-AUTH-500` predate Google sign-in and were undocumented drift, closed here.
...
| `SAMO-AUTH-005` | 401 + clear-cookie | Stateless session outlived its tenant (§5.14 erasure, or a recreated dev DB) | "You've been signed out. Please sign in again." | Re-authenticate |
| `SAMO-AUTH-006` | `302 → /auth?error=…` | User cancelled at Google's consent screen (`error=access_denied`) | "Sign-in cancelled. Choose a way to sign in below." | None — rendered in an info tone, not as an error; offer both sign-in options again |
...
| `SAMO-AUTH-008` | `302 → /auth?error=…` | Google-side or token/ID-token failure (token exchange, JWKS, signature, `iss`/`aud`/`azp`/`exp`/`iat`/`nonce`) | "Google couldn't sign you in right now." | Retry, or use magic link |
| `SAMO-AUTH-009` | `302 → /auth?error=…` | `email_verified` is not boolean `true` on the verified ID token — create nothing, link nothing, mint no cookie | "Your Google account's email isn't verified." | Verify with Google, or use magic link |
...
| `SAMO-AUTH-500` | 500 | Infra/provisioning failure **after** a valid credential verified (e.g. the pre-tenant `INSERT INTO tenants` errors — #180). On the magic-link path the single-use link is left OUTSTANDING, so the user can simply click it again | "Something went wrong on our end — please try again." | Retryable — try again |
...
**Google sign-in delivery (S5-1).** `retryable` is `true` for `SAMO-AUTH-006`/`007`/`008` (try again, or use magic link) and `false` for `009`/`010` — clicking again changes nothing until Google verifies the address, or until an operator configures credentials on that environment. `SAMO-AUTH-004` is **reused** on both Google legs (`start` and `callback` each hold their OWN 20/hr/IP bucket, separate from the magic-link per-IP budget, so neither credential path can exhaust the other's) and is delivered there as `302 → /auth?error=SAMO-AUTH-004`, **not** as the 429 + `Retry-After` that `POST /auth/magic-link` returns — a 429 carrying a `Location` is a dead end for a browser that arrived by clicking a link. Its row above keeps the 429 of its original, JSON-answering call site.

**Contract:** every code that can be delivered as `/auth?error=<CODE>` MUST have a row in the web code→copy map, rendering the string in the table above **verbatim** — the app-api and web copies are asserted byte-for-byte against each other, because a silent edit to either is user-visible drift.

This invariant **holds today** and is enforced mechanically, not by review (issue #219, PR #230). `apps/web/lib/authErrors.ts` no longer declares its own code list: it *derives* one as `Exclude<AppApiAuthErrorCode, ServerInternalAuthErrorCode>` from the app-api's `AuthErrorCode`, and keys its copy map as a `Record` over that derived union. Adding a code to the app-api therefore breaks `tsc --noEmit` in the web until it has copy or is explicitly declared server-internal, and a test asserts the exclusion list *literally* so it cannot be quietly grown to silence that build error. What is enforced is exactly "every app-api `SAMO-AUTH-*` code except the declared server-internal ones has copy" — the compiler cannot itself tell which codes reach `errorRedirect`, so the exclusion list is the human judgement the machine then holds you to.

`SAMO-AUTH-005` is the **one** documented exclusion: it is a 401 + clear-cookie on a tenant-scoped route and is never a redirect, so copy for it would describe a screen that cannot happen. `SAMO-AUTH-500` is *not* excluded — like `SAMO-AUTH-004` it is dual-delivered (a 500 + JSON body on the magic-link leg; `302 → /auth?error=SAMO-AUTH-500` when Google's token endpoint or identity provisioning fails), and its row above keeps the 500 of its original, JSON-answering call site for the same reason `004`'s keeps its 429.

None of `SAMO-AUTH-006`–`010` distinguishes "this email exists in our DB" from "it does not" — the split is "your browser/tab went stale" vs "Google's side failed" vs "your own Google email is unverified", which is not a fact about our user table and materially changes what the user does next. Sub-reasons *inside* each bucket are indistinguishable, and Google's `error` / `error_description` is **never** reflected into a URL, a response body, or a rendered log.
...
> **Amendment (S5-1 item 7, issue #222).** For the first full week after Google sign-in ships, the **≥ 0.5 target is judged against `method="magic_link"`** — read from `samograph_activation_w1_by_method` — and the blended `samograph_activation_w1` is reported but **not** targeted, then re-baselined. One-click signup raises the denominator faster than the numerator, so the headline metric can fall while the product improves.
