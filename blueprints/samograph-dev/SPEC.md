...
> **Amendment (S5-1).** Post-v1, `GET /auth/google/start` + `GET /auth/google/callback` add Google (OIDC) sign-in alongside the magic link, terminating in the **same** `samo_session` cookie this section mints (one cookie mint site, no second session shape). Identity is keyed on `(provider, provider_subject)` in the privileged `user_identities` table (migration `0011`), **not** on `users.email` — which becomes **immutable after creation** (no statement anywhere mutates it: the only `UPDATE` naming the column is `apps/app-api/auth/pg-user-store.ts`'s `ON CONFLICT (email) DO UPDATE SET email = EXCLUDED.email`, a provable no-op self-assignment — the conflict target *is* the unique `email` — written so `RETURNING` fires on the conflict branch); `email_verified === true` on the locally verified ID token is a hard precondition before any store is touched; linking a Google identity to an existing account is silent and therefore sends a one-time notification email. `GET /auth/providers` is the sole gate on whether the Google button renders on a deployment (it reports `{"google": false}` on branch previews, by design). See `SPEC.amendments.md` → S5-1.
