...
> **Amendment (S5-1).** Post-v1, `GET /auth/google/start` + `GET /auth/google/callback` add Google (OIDC) sign-in alongside the magic link, terminating in the **same** `samo_session` cookie this section mints (one cookie mint site, no second session shape). Identity is keyed on `(provider, provider_subject)` in the privileged `user_identities` table (migration `0011`), **not** on `users.email` — which becomes **immutable after creation** (no `UPDATE users SET email` exists anywhere); `email_verified === true` on the locally verified ID token is a hard precondition before any store is touched; linking a Google identity to an existing account is silent and therefore sends a one-time notification email. `GET /auth/providers` is the sole gate on whether the Google button renders on a deployment (it reports `{"google": false}` on branch previews, by design). See `SPEC.amendments.md` → S5-1.
...
**Contract:** every code that can be delivered as `/auth?error=<CODE>` MUST have a row in the web code→copy map, rendering the string in the table above **verbatim** — the app-api and web copies are asserted byte-for-byte against each other, because a silent edit to either is user-visible drift. `SAMO-AUTH-005` is deliberately **excluded** from that map: it is a 401 + clear-cookie on a tenant-scoped route and is never a redirect.
