  ...
  it("apply cleanly on a fresh DB and are idempotent (exact applied set)", async () => {
    ...
    await sql.unsafe(`
      DROP TABLE IF EXISTS
        webhook_events, transcripts, tokens, workers, audit_log, deleted_calls, settings, calls, calendar_events, calendar_connections, tenants, user_identities, users, regions, magic_links, schema_migrations
      CASCADE;
